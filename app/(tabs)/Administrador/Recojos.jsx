...
const RecojoActivoList = ({ route }) => {
  ...
  const handleEnviarRecojoId = async () => {
    try {
      const response = await fetch('https://verdeulima.azurewebsites.net/back/consultar_recojo', {
        method: 'POST',
        headers: {
  ...
  const handleRetrocederEstado = async () => {
    try {
      const response = await fetch('https://verdeulima.azurewebsites.net/back/retroceder_estado', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
