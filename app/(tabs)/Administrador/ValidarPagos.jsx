...
const ValidarPagos = () => {
  ...
  useEffect(() => {
    const fetchPagosData = async () => {
      try {
        const response = await fetch(`https://verdeulima.azurewebsites.net/back/listar_pagos_no_validados/`);
        if (!response.ok) {
  ...
  const validarPago = async (pagoId) => {
    try {
      const response = await fetch(`https://verdeulima.azurewebsites.net/back/validar_pago/${pagoId}/`, {
        method: 'POST',
      });
  ...
  const handleEnviarPDF = async () => {
    try {
      ...
      const response = await fetch('https://verdeulima.azurewebsites.net/back/enviar_PDF', {
        method: 'POST',
        headers: {
