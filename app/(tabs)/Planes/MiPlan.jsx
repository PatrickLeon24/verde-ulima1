...
const PantallaPlan = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchPlanActual = async () => {
      if (userData) {
        try {
          const response = await fetch(`https://verdeulima.azurewebsites.net/back/planes/${userData.usuario_id}/`);
          if (!response.ok) {
  ...
  useEffect(() => {
    const fetchPlanes = async () => {
      try {
        const response = await fetch('https://verdeulima.azurewebsites.net/back/planesRecojo');
        if (!response.ok) {
